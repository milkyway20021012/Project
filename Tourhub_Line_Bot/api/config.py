...
# 集合時間模式
MEETING_TIME_PATTERN = r'集合.*時間|時間.*集合|集合.*\d{1,2}:\d{2}|明天.*集合|今天.*集合|下午.*集合|上午.*集合|晚上.*集合|凌晨.*集合|集合.*下午|集合.*上午|集合.*晚上|集合.*凌晨|\d{1,2}:\d{2}.*集合|\d{1,2}點\d{1,2}分.*集合|\d{1,2}點.*集合|幾點.*集合|幾點.*見|約在.*|.*集合|.*見面'
